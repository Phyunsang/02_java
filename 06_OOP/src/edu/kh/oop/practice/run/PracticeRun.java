...
/*
 ...
 캡슐화 : 속성과 기능을 하나로 묶어 관리하며, 데이터의 직접접근제한을 원칙으로함
 		대신 간접접근 할 수 있도록 클래스 내부에 gettet/setter 메서드를 작성함.
 
 캡슐화 원칙 : 1) 멤버변수에 대한 접근 권한은 private을 원칙으로 함.
 			   2) 멤버변수에 대한 연산처리를 목적으로 한 메서드는 클래스 내부에 작성함.
 			   3) 멤버 메서드는 클래스 밖에서 접근 가능하도록 public 으로 설정함.
 
 메서드/기능의 정의란 : 해당 메서드가 어떤 기능을 수행/실행할 것인지 작성하는 것.(정의하는 것)
 ...
 위 메서드 상세히 해석 : 어디서든 접근 가능한(public) ex1 이라는 메서드를 정의하고,
